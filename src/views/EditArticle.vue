...
async function getArticleData() {
  try {
    ...
    if (response.status === 200 && response.data) {
      ...
      formData.body = body || "";
      formData.tagString = (tagList || []).join(", ");
    }
  } catch (error) {
    console.error(error);
    showToast("Failed to load article.", "error", true);
  }
}

// Called when ArticleForm successfully updates the article
async function onArticleUpdated(article) {
  showToast("Article updated — redirecting...", "success", false);

  // Use the NEW slug from the response, not the old one
  const newSlug = article?.slug || articleSlug.value;

  setTimeout(() => {
    hideToast();
    router.push({ name: "articles", params: { slug: newSlug } }).catch(() => {
      router.push(`/articles/${newSlug}`).catch(() => {});
    });
  }, 700);
}

// Called when ArticleForm emits an error
function onErrorFromForm(message) {
  showToast(message || "Failed to update article.", "error", true);
}
